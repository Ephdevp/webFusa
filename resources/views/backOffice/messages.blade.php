                        ...
                        <div class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#flush-collapseOne{{ $temp->id }}" aria-expanded="false"
                            ...
                            <div id="flush-collapseOne{{ $temp->id }}" class="accordion-collapse collapse ms-3"
                                data-bs-parent="#accordionFlushExample">
                                <div class="row">
                                    <h6 class="mt-3"><i class="bi bi-telephone"></i> Teléfono:<small
                                            class="ms-2">{{ $temp->telefono }}</small></h6>
                                    <h6><i class="bi bi-envelope-at"></i> Email:<small
                                            class="ms-2">{{ $temp->email }}</small></h6>
                                    @if ($temp->leido == false)
                                        <a class="text-success text-decoration-none"
                                            href="{{ route('message.update', $temp->id) }}"><i class="bi bi-check-lg"></i>
                                            Marcar cómo leido</a>
                                    @endif
                                </div>
                                <div class="accordion-body border mt-3 mb-5 shadow">
                                    <h5><i class="bi bi-envelope-paper"></i> Mensaje:</h5>
                                    <p class="ms-2">{{ $temp->mensaje }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                @if ($temp->leido == 1)
                    <div class="col-1">
                        <form method="POST" action="{{ route('message.delete', $temp->id) }}">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn mt-2" title="Eliminar"><i
